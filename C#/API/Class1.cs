...
//the Receive: { [character] -> Event } "convert user input to events"
    public class TheEngine<Event> : Engine<Event>
    {
        private Behavior<Event> b;
        public Job raise(Event e) { return delegate() { b.handle(e); }; }
        public Job delay(double interval, Event e) { return delegate() { b.handle(e); }; } //TODO
    }

    public delegate Job Output(IahaArray<char> output);

    public interface IBehaviorParams<Event>
    {
        IahaArray<char> input();
        Output output();
        Engine<Event> engine();
    }
    ...
    public class Application<Event> : AhaModule
    {
        class behavior : Behavior<Event>
        {
            ...
            public behavior(IahaArray<char> inp, Output outp, Engine<Event> eng) { input = inp; outp = output; engine = eng; }
        }
        ...
        public GetBehavior<Event> Behavior() { return delegate(IBehaviorParams<Event> param) { return new behavior(param.input(), param.output(), param.engine()); }; }
