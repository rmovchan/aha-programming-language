...
This module introduces floating-point numbers and basic operations on them. Note that the Float type is opaque and therefore it is up to the implementer what internal representation to choose, e.g. 4-byte, 8-byte or 10-byte.

In addition, operations on matrices are introduced.
...
# Examples #
```
( @Math.FloatToString(value, fmt) 
where
    all
        value = (sin(Pi / (~float 6)) with @Math)!
        fmt = [ general: [ period: $.; ]; ]!
    end
) ` returns "0.5" `

(~struc Round(~float (9 / 10), 0) with @Math) ` returns [ num: 1; den: 1; ] `
